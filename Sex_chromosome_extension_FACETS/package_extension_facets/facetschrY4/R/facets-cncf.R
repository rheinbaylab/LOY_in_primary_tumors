...
fitcncf <- function(out, dipLogR=0, nX=23) {
    ...
    cncf$ocn <- out1$ocn[ii] # add ocn information
    cncf$tcn.1.XY.float = 0
    cncf$tcn.1.before = out1$tcn[ii]
    ...
    # edited by MQ: revise if the subject is male/female/LOY
    for (num_sex_chr in 23:24){
    ii <- which(cncf$chrom==num_sex_chr)
    if (length(ii) > 0) {
        # male if nhets on X chromosome is less than 1% of num.mark
        if (sum(cncf$nhet[ii])/sum(cncf$num.mark[ii]) < 0.01) {
            # cncf$tcn[ii] <- ceiling(cncf$tcn[ii]/2) # MQ
            cncf$tcn.1.before[ii] <- round(cncf$tcn.1.before[ii]/2) # MQ
            cncf$tcn[ii] <- round(((cncf$ocn[ii] -2)/cncf$cf[ii] + 2)/2)
            cncf$tcn[ii[cncf$tcn[ii] < 0]] = 0
	    cncf$tcn.1.XY.float[ii] <- ((cncf$ocn[ii] -2)/cncf$cf[ii] + 2)/2
	    # cncf$tcn[ii] <- round(cncf$ocn[ii]/2,0)
            cncf$lcn[ii] <- 0
            # set cf to be 1 if tcn==1
            cncf$cf[ii][cncf$tcn[ii]==1] <- NA
        }
    }
}
...
fitcncf0 <- function(out, dipLogR=0) {
    ...
    out$ocn <- 2^(1 + out$cnlr.median - dipLogR) # for estimate the copy
    ocn0 <- out$ocn
...
# can add tcn & cf for 23/24 if sample are male: # add by MQ
