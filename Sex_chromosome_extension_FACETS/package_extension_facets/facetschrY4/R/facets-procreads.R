...
procSnps <- function(rcmat, ndepth=35, het.thresh=0.25, snp.nbhd=250, nX=23, unmatched=FALSE, ndepthmax=1000) {
    ...
    # chromlevels <- c(1:(nX-2),"X",'Y') # MQ: add Y
...
# ndepthmax=1000
    ...
    chromlevels <- rcmat$Chromosome %>% unique()
    ...
    # rcmat$chrom <- as.numeric(ordered(rcmat$chrom, levels = chromlevels)) # this code is too tricky
...
counts2logROR <- function(out, gbuild, unmatched=FALSE, ugcpct=NULL, f=0.2) {
   ...
   # f = 0.2
    ...
    chrs = unique(out$chrom)
    nchr <- length(chrs) # IMPACT doesn't have X so only 22
    # assign(out, out_new, envir = .GlobalEnv)
    for (i in 1:nchr) {
        # ii <- which(out$chrom==i)
        ii <- which(out$chrom==chrs[i])
        # allow for chromosomes with no SNPs i.e. not targeted
        if (length(ii) > 0) {
            if (gbuild == "udef") {
                out$gcpct[ii] <- getGCpct(chrs[i], out$maploc[ii], gbuild, ugcpct)
            } else {
                out$gcpct[ii] <- getGCpct(chrs[i], out$maploc[ii], gbuild)
            }
        }
    }
    ...
    tscl <- sum(ncount)/sum(tcount) # ratio of total count in gc position between normal and tumor samples
    gcb <- lowess(pctgc, log2(tcount*tscl)-log2(ncount), f=f) # gc biase;
