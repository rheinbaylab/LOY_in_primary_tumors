...
segsnps <- function(mat, cval=25, hetscale=FALSE, delta=0) {
    ...
    mat <- mat[ii, c("chrom","cnlr","valor","het")] # problem: removed position, which means, might not be enough for segmentation if distance is very large
    ...
    # chrs <- 1:nchr # edit by MQ
    chrs = mat$chrom %>% unique()
    ...
    # for(i in chrs) {
        ...
        if (nrow(genomdat) == 0) {
            ...
            if (i >23){
              tmp <- fit.cpt.tree(genomdat, cval=50, hscl=hscl, delta=delta)
            }else{ tmp <- fit.cpt.tree(genomdat, cval=cval, hscl=hscl, delta=delta)}
