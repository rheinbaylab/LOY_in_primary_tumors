...
emcncf=function(x,trace=FALSE,unif=FALSE,min.nhet=15,maxiter=10,eps=1e-3){
  ...
  logORvar=jointseg$lorvar # logR variance
  logOR2var=logOR^2/logORvar # logor2 variance
  logORvar.clust=by(logORvar,jointseg$segclust,function(x)mean(na.omit(x))) # mean value of logORvar in each clust
  ...
  # homdel=which(major.lsd==0&major.lsd==0) # is there wrong with the code? MQ
  homdel=which(major.lsd==0&minor.lsd==0) # is there wrong with the code? MQ
  ...
  while(dif>eps && iter<maxiter) {
    ...
    if (sum(is.na(rhov)) >0) {dif = 0 }else   { dif = quantile(abs(rhov-rhov.old),0.9,na.rm=TRUE)} # edit by MQ
...
# MQ: new for X/Y and PAR
  ...
  if(male){
    ...
    t.em[chr==24]=round(t.em[chr==24]/2,0)
    minor.em[chr==24]=NA
    normalY=which(t.em[chr==24]==1)
    if(any(normalY))rhov.em[chr==24][normalY]=1
  ...
  # edit by MQ for following script
  oct.em.tmp= (2^(seglogr.adj+1)-2*(1-rho))/rho
  tcn.em.before = t.em
  if(male){

    oct.em.tmp[chr==23] = oct.em.tmp[chr==23]/2
    oct.em.tmp[chr==24] = oct.em.tmp[chr==24]/2
    t.em[chr==23] = round( oct.em.tmp[chr==23])
    t.em[chr==24] = round( oct.em.tmp[chr==24])
  }

    out1=data.frame(seg[,1:9],start=startseq,end=endseq,cf.em=rhov.em,tcn.em=t.em,lcn.em=minor.em, seglogr.adj = seglogr.adj, oct.em = oct.em.tmp, tcn.em.before = tcn.em.before ) # edit by MQ
    # out1=data.frame(seg[,1:9],start=startseq,end=endseq,cf.em=rhov.em,tcn.em=t.em,lcn.em=minor.em)
