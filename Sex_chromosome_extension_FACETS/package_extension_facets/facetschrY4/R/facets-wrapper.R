...
preProcSample <- function(rcmat, ndepth=35, het.thresh=0.25, snp.nbhd=250, cval=25, deltaCN=0, gbuild=c("hg19", "hg38", "hg18", "mm9", "mm10", "udef"), ugcpct=NULL, hetscale=TRUE, unmatched=FALSE, ndepthmax=1000) {
    ...
    if (gbuild == "udef") {
        if (missing(ugcpct)) {
            ...
            # nX <- length(ugcpct) # MQ
          nX <- sum(df_genomesize$if_auto) + 1
        }
    }
    pmat <- procSnps(rcmat, ndepth, het.thresh, snp.nbhd, nX, unmatched, ndepthmax)
    if (gbuild == "udef") {
       dmat <- counts2logROR(pmat[pmat$rCountT>0,], gbuild, unmatched, ugcpct) # by MQ
    } else {
      dmat <- counts2logROR(pmat[pmat$rCountT>0,], gbuild, unmatched) # by MQ
    }
...
# dipLogR = NULL
    ...
    # chrs <- x$chromlevels
    chrs <- unique(jseg$chrom)
    nchr <- length(chrs)
    # get chromlevels from chrs
    # chromlevels <- c(1:(nX-2), "X",'Y')[chrs]
    chromlevels = x$chromlevels
    # chromlevels <- c(1:(nX-1), "X")[chrs] # MQ: change the chromlevels
...
logRlogORspider <- function(cncf, dipLogR=0, nfrac=0.005) {
    ...
    legend(-1, 5.25, paste("chr", c(1:22, "X","Y"), sep=""), ncol=4, pch=10, col=chrcol[1:23], cex=0.65) # MQ: add 'Y' to plot the data
    # legend(-1, 5.25, paste("chr", c(1:22, "X"), sep=""), ncol=4, pch=10, col=chrcol[1:23], cex=0.65) # MQ: add 'Y' to plot the data
}
